...
class PlusOperation implements OperationInterface
{
    ...
    {
        $this->loggerInterface->write("Started plus operation");
        ...
        $this->loggerInterface->write("Finished plus operation");
    }
    ...
    public function validate(int $number1, int $number2): bool
    {
        ...
        if ($number1 < 0 && (abs($number1) > $number2)) return false;
        if ($number2 < 0 && (abs($number2) > $number1)) return false;
